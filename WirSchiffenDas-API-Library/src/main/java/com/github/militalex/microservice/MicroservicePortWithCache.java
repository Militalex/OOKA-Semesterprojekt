...
import com.github.militalex.microservice.util.IResultCaching;
import com.github.militalex.microservice.util.ListResultCache;
...
public abstract class MicroservicePortWithCache extends MicroservicePort {
    ...
    public MicroservicePortWithCache(Microservice microservice, MicroserviceService service,
    ...
    private void receiveAnalysisRequestFromEngMng(@Header(KafkaHeaders.RECEIVED_KEY) int sessionId){
        microservice.getOptionalEquipments().stream()
        ...
        CompletableFuture<Void> futureResult = CompletableFuture.runAsync(() -> {
            ...
            for (int i = 0; i < maxSeconds; i++){
                if (microservice.getOptionalEquipments().stream()
